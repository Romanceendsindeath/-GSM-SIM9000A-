...
void UART2SendString(u8 *cmd,u16 len)
{
	u16 i=0;	
	while((*(cmd+i)!=0)&&(i<len))
	{
			UART2SendByte(*(cmd+i));i++;
	}
}
//通用定时器中断初始化
...
{ 	
	...
	{	 			   
		...
		TIM4_Set(0);			//关闭TIM4  
		UART3SendString(USART2_RX_BUF,USART2_RX_STA&0x7fff);	//输出LOG信息
		if(!USART2_RX_REC_ATCOMMAD)	
		{
			USART2_RX_BUF[USART2_RX_STA&0X7FFF]=0;//添加结束符
			USART2_RX_STA=0;
			if(strstr((char*)USART2_RX_BUF,"CMT:")!=NULL)	Flag_Rec_Message=1;//检测是否是来电话了
		}
